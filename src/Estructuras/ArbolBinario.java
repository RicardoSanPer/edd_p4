...
public abstract class ArbolBinario<T> implements Collection<T> {
    ...
    */
	...
	//Iterator<T> iter = coleccion.iterator();
    ...
    */
	...
	else
	{
	    ...
	    //Si el elemento ya se encuentra en el arbol, lo omite.
	    if(temp.elemento == e){return;}
	    //navega al arbol de menor altura
	    while(temp.hayIzquierdo() && temp.hayDerecho())
	    {
		if(temp.izquierdo.altura() <= temp.derecho.altura())
		{
		    temp = temp.izquierdo;
		}
		else
	    ...
	    //Coloca en el izuierdo si no hay izquierdo
	    if(temp.izquierdo == null)
	    {
		temp.izquierdo = new Vertice(e);
		temp.izquierdo.padre = temp;
	    }
	    //coloca en el derecho si no hay derecho
	    else
	    {
		temp.derecho = new Vertice(e);
